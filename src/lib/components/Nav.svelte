<script lang="ts">
  ...
  onNavigate(({ from, to, type }) => {
    if (from && to && browser) { // Only run in browser environment
  ...
  function createHoverParticles(container: HTMLElement, sourceElement: HTMLElement, count: number = 5) {
        ...
        if (navIconElements && navIconElements.length > 0) {
          ...
          if (navTextElements && navTextElements.length > 0) {
            ...
            navTexts.forEach((text, index) => {
              ...
              }, (index * 300) + 150); // Slightly offset from icons
            });
          }
        }
...
<style>
  ...
  :global(.nav-hover-glow) {
    ...
    height: 40px;
    border-radius: 50%;
    background: radial-gradient(circle, rgba(96, 165, 250, 0.9) 0%, rgba(139, 92, 246, 0.4) 40%, rgba(96, 165, 250, 0) 70%);
    transform-origin: center;
    pointer-events: none;
    z-index: 1;
    opacity: 0;
  ...
  @keyframes textPulse {
    0% { opacity: 0.9; text-shadow: 0 0 5px rgba(96, 165, 250, 0.3); }
    50% { opacity: 1; text-shadow: 0 0 15px rgba(96, 165, 250, 0.7); }
    100% { opacity: 0.9; text-shadow: 0 0 5px rgba(96, 165, 250, 0.3); }
  }
