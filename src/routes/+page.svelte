<script lang="ts">
	...
	onMount(() => {
		// Staggered animation for the hero section elements
		animate(
			'.hero-element', 
			{ 
				opacity: [0.8, 1],  // Start partially visible
				y: [30, 0]
			},
			{ 
				delay: stagger(0.1),
				duration: 0.6,
				easing: spring({ stiffness: 80, damping: 20 })
			}
		);
		...
		animate(
			...
			{ 
				delay: stagger(0.03),
				duration: 0.5,
				easing: spring({ stiffness: 120, damping: 12 })
			}
		);
...
<section class="py-12 bg-[#1A202C] text-gray-100" bind:this={heroSection}>
	<div class="container mx-auto px-4">
		<div class="flex flex-col items-center">
			<div class="w-full max-w-3xl mb-8 text-center">
				<h1 class="text-4xl md:text-5xl font-bold mb-4 hero-element">Hi, I'm <span class="text-blue-400">Szu-Han Chou (Hank)</span></h1>
				<h2 class="text-2xl md:text-3xl text-gray-300 mb-6 hero-element">Software Developer</h2>
				<p class="text-lg text-gray-200 mb-8 hero-element">
				...
				<div class="flex flex-wrap gap-3 mb-8 justify-center">
					{#each skills as skill}
						<span class="bg-[#2D3748] text-blue-300 px-3 py-1 rounded-full text-sm border border-gray-700 hover:border-blue-500 transition-colors skill-tag">{skill}</span>
					{/each}
				</div>
				<div class="flex gap-4 justify-center">
					<a href="/portfolio" class="bg-blue-700 hover:bg-blue-600 text-white px-6 py-3 rounded-lg transition-colors shadow-lg hero-button">
						View My Work
					</a>
					<a href="/services" class="border border-blue-500 text-blue-400 hover:bg-[#2D3748] px-6 py-3 rounded-lg transition-colors shadow-lg hero-button">
						My Expertise
					</a>
				</div>
			</div>
...
<section class="py-12 bg-[#1E2433] text-gray-100" bind:this={aboutSection}>
	<div class="container mx-auto px-4">
		<h2 class="text-3xl font-bold mb-8 text-center text-blue-400 about-element">About Me</h2>
...
<style>
	...
	/* Make content visible initially, but still animatable */
	.hero-element, .skill-tag, .hero-button, .about-element {
